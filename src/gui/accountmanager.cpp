...
const QString legacyNCUser()
{
    return QStringLiteral("webflow_user");
}
...
AccountPtr AccountManager::loadAccountHelper(QSettings &settings)
{
    ...
    // If we have a legacy NextCloud webflow_user key, use that as username if it wasnt found.
    if (acc->_davUser.isEmpty()) {
        acc->_davUser = settings.value(legacyNCUser()).toString();
    }
