        ...
        while (QPointer<QTcpSocket> socket = _server.nextPendingConnection()) {
            ...
            QObject::connect(socket.data(), &QIODevice::readyRead, this, [this, socket] {
                ...
                QObject::connect(job, &SimpleNetworkJob::finishedSignal, this, [this, socket](QNetworkReply *reply) {
                    ...
                    const QString refreshToken = data[QStringLiteral("refresh_token")].toString();
                    ...
                    if (reply->error() != QNetworkReply::NoError || jsonParseError.error != QJsonParseError::NoError
                        ...
                        emit result(Error);
                        return;
                    } else if (tokenType == QLatin1String("app-password")) {
                        finalize(socket, accessToken, refreshToken, user, messageUrl);
                        return;
                    } else if (tokenType != QLatin1String("Bearer")) {
                        QString errorReason = tr("Unsupported token type: %1").arg(tokenType);
                        httpReplyAndClose(socket, QByteArrayLiteral("500 Internal Server Error"),
                            tr("<h1>Login Error</h1><p>%1</p>").arg(errorReason).toUtf8());
                        emit result(Error);
                    }
    ...
    const QString &refreshToken, const QString &user, const QUrl &messageUrl)
{
    ...
    if (refreshToken == QStringLiteral("")) {
        emit result(LoggedInAppPassword, user, accessToken, refreshToken);
    } else {
...
{
    const QUrl requestTokenUrl = _tokenEndpoint.isEmpty() ? Utility::concatUrlPath(_account->url(), QStringLiteral("/request-client-token/finalize")) : _tokenEndpoint;
    QNetworkRequest req;
...
QUrl OAuth::authorisationLink() const
{
    ...
    const QUrl url = _authEndpoint.isValid()
        ...
        : Utility::concatUrlPath(_account->url(), QStringLiteral("/request-client-token/authorize"), query);
    return url;
}
...
{
    qCDebug(lcOauth()) << "Checking URL for validity:" << url << url.host(QUrl::PrettyDecoded);

    if (url.host(QUrl::PrettyDecoded) == QStringLiteral("test.stack.us")) {
        qCDebug(lcOauth()) << "Allowing STACK test URL:" << url;
        return true;
    }
