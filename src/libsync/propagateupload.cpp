...
void PropagateUploadFileCommon::commonErrorHandling(AbstractNetworkJob *job)
{
    ...
    if (_item->_httpErrorCode == 507) {
        ...
        emit propagator()->insufficientRemoteStorage();
    } else if (_item->_httpErrorCode == 405) {
        status = SyncFileItem::FileIgnored;
        _item->_instruction = CSYNC_INSTRUCTION_IGNORE;
        errorString = tr("Upload of file rejected by server");
    } else if (_item->_httpErrorCode == 491) {
        status = SyncFileItem::FileIgnored;
        _item->_instruction = CSYNC_INSTRUCTION_IGNORE;
        errorString = tr("Invalid file path");
    } else if (_item->_httpErrorCode == 492) {
        status = SyncFileItem::FileIgnored;
        _item->_instruction = CSYNC_INSTRUCTION_IGNORE;
        errorString = tr("File is sparse (contains large amounts of zeroes)");
    }
