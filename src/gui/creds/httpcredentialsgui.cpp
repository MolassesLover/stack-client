    ...
    const QString &token, const QString &refreshToken)
{
    ...
    case OAuth::Error:
        ...
        return;
    case OAuth::LoggedIn:
        qCInfo(lcHttpCredentialsGui) << "Logged in " << _account->davUrl();
        break;
    case OAuth::LoggedInAppPassword:
        // We received an app password, so set type accordingly
        _authType = DetermineAuthTypeJob::AuthType::Basic;
        _user = user;
        _refreshToken.clear();
        qCInfo(lcHttpCredentialsGui) << "Setting auth type for " << _account->davUrl() << " to " << _authType;
        break;
    }
