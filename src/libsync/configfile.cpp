...
const QString updateSegmentC() { return QStringLiteral("updateSegment"); }
...
{
    QSettings settings(configFile(), QSettings::IniFormat);
    return settings.value(chunkSizeC(), 8 * 1024 * 1024).toLongLong(); // default to 8 MiB
}

qint64 ConfigFile::maxChunkSize() const
{
    QSettings settings(configFile(), QSettings::IniFormat);
    return settings.value(maxChunkSizeC(), 256 * 1024 * 1024).toLongLong(); // default to 256 MiB
}

qint64 ConfigFile::minChunkSize() const
{
    QSettings settings(configFile(), QSettings::IniFormat);
    return settings.value(minChunkSizeC(), 8 * 1024 * 1024).toLongLong(); // default to 8 MiB
}
